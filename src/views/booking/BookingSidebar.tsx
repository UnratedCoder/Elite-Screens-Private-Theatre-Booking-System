...
export default function BookingSidebar() {
  ...
  return (
    <div className="lg:col-span-4 lg:sticky lg:top-24 bg-white rounded-2xl border border-gray-200 p-4 sm:p-5 shadow-sm space-y-4 sm:space-y-5">
      ...
      <div className="space-y-2.5 text-xs border-t border-gray-100 pt-4">
        ...
        <div className="flex justify-between items-start gap-2">
          <span className="text-gray-400 font-medium shrink-0">🕒 Slot</span>
          <span className="font-bold text-gray-800 text-right text-[11.5px] leading-tight" title={`${formatDateLabel(date)}, ${slotTime}`}>
            {formatDateLabel(date)}, {slotTime}
          </span>
        </div>
        <div className="flex justify-between items-center">
          <span className="text-gray-400 font-medium">
            {step >= 3 ? "🏨 Slot Price" : "💰 Price"}
          </span>
        ...
        {step >= 3 && kids > 0 && (
          <div className="flex justify-between items-center">
            <span className="text-gray-400 font-medium">👶 Kids (3-10 years)</span>
            <span className="font-bold text-gray-800">
              {kids} ({kids} * ₹200) ₹{kidsPrice}
            </span>
          </div>
        )}
        ...
        {step >= 4 && occasion && (
          <div className="space-y-1.5 bg-orange-50/20 p-2.5 rounded-xl border border-orange-100/50 mt-1">
            ...
            {/* Display dynamic names in summary */}
            ...
            {occasionMessage && (
              <div className="flex justify-between items-start gap-2 text-[10.5px]">
                <span className="text-gray-400 font-medium shrink-0">💬 Message</span>
                <span className="font-bold text-gray-700 text-left break-words max-w-[170px]" title={occasionMessage}>
                  {occasionMessage}
                </span>
              </div>
            )}
            ...
            <ul className="space-y-1.5">
              {cart.map((item) => (
                <li key={item.id} className="flex justify-between items-center text-[11px] text-gray-600 font-semibold bg-gray-50 rounded-lg p-1.5 border border-gray-100">
                  <span className="flex-1 mr-2 truncate text-left" title={item.name}>
                    {item.name}
                  </span>
      ...
      {validationError && (
        <div className="bg-red-50 text-red-600 border border-red-100 rounded-xl p-3 text-center text-xs font-semibold animate-pulse">
          ⚠️ {validationError}
        </div>
      )}
      ...
      <button
        ...
        className="w-full rounded-xl bg-[#C85A17] hover:bg-[#A04000] text-white py-3 text-xs font-bold uppercase tracking-wider shadow-md cursor-pointer transition-all hover:scale-102 flex items-center justify-center gap-1.5 select-none"
      >
